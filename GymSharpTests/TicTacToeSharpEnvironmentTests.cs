...
namespace GymSharp.Tests
{
    ...
    public class TicTacToeSharpEnvironmentTests
    {
        ...
        public void Setup()
        {
            ...
            np.random.seed(0);
            dynamic rng = np.random.RandomState();
            env = new TicTacToeSharpEnvironment(rng);
            var ts = env.reset();
            assert_array_equals(new int[3, 3], ts.observation);
        ...
        public void test_legal_actions()
        {
            ...
            assert_array_equals(expected_result, env_result);
        }


        private void assert_array_equals(int[,] empty, int[,] observation)
        {
            var a = empty.Flatten();
            var b = observation.Flatten();
        ...
        public void test_step_win()
        {
            ...
            assert_array_equals(expected, ts.observation);
