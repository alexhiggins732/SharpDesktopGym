...
class CartPoleSharpEnvironment(py_environment.PyEnvironment):
  ...
  def __init__(self, rng: np.random.RandomState=None, discount=1.0):
     super(CartPoleSharpEnvironment, self).__init__()
     sharp_env = CartPoleSharpEnvironment(rng)
     #x_threshold * 2, float.MaxValue, theta_threshold_radians * 2, float.MaxValue

     self.high = np.array([x_threshold * 2, sys.float_info.max, theta_threshold_radians * 2, sys.float_info.max]);
     self.low = -self.high
     sharp_env.action_spec = BoundedArraySpec(tf.TensorShape((2,)), np.float32, minimum=self.low, maximum=self.high)
     sharp_env.observation_spec = BoundedArraySpec(tf.TensorShape((3, 3)), np.float32, minimum=0, maximum=2)
