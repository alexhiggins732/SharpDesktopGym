...
use_sharp = True
if use_sharp:
    from TicTacToeSharpGenericEnvironmentWrapper import TicTacToeGenericEnvironment
    print ("Using TicTacToe C# Environment")
else:
    from tic_tac_toe_environment import TicTacToeEnvironment
    print ("Using TicTacToe Py Environment")
...
class TicTacToeEnvironmentTest(test_utils.TestCase):
  ...
  def setUp(self):
    ...
    self.env = TicTacToeGenericEnvironment()
    ts = self.env.reset()
    py_array = netArrays.asNumpyTupleArray(ts.observation)
    np.testing.assert_array_equal(np.zeros((3, 3), np.int32), py_array)
  ...
  def test_legal_actions(self):
    ...
    env_result = self.env._legal_actions(states)
    expected_result = [(0, 0), (0, 1), (0, 2), (1, 1), (1, 2), (2, 2)]
    self.assertEqual(expected_result,env_result)
  ...
  def test_opponent_play_random(self):
    self.env = TicTacToeGenericEnvironment(rng=np.random.RandomState(0))
    s = set()
  ...
  def test_step_win(self):
    draw_reward  = self.env.REWARD_DRAW_OR_NOT_FINAL
    self.env.set_state(TimeStep(StepType.MID, draw_reward,
  ...
  def test_step_loss(self):
    self.env.set_state(TimeStep(StepType.MID,  self.env.REWARD_DRAW_OR_NOT_FINAL,
                 self.discount, np.array([[2, 2, 0], [0, 1, 1], [0, 0, 0]])))
  ...
  def test_step_illegal_move(self):
    ...
    np.testing.assert_array_equal([[2, 2, 0], [0, 1, 1], [0, 0, 0]],
                                  ts.observation)
    self.assertEqual(StepType.LAST, ts.step_type)
    self.assertEqual(self.env.REWARD_ILLEGAL_MOVE, ts.reward)
